...
//       nextDisabled={!location?.coordinates}
//       onNext={handleNext}
//     >
...
//         Guests will see an approximate location before booking.
//       </p>
...
export default function LocationPage() {
  const router = useRouter();
  const { data, updateData ,goToNextSubStep ,goToPrevSubStep } = serviceOnboarding();
...
const hasLocation = Boolean(
  ...
  const backendCoords = data.location?.coordinates;
  // Location state
  const [location, setLocation] = useState<LocationDetails>({
    ...
    coordinates:
  Array.isArray(backendCoords) && backendCoords.length === 2
    ? {
        lat: backendCoords[0],
        lng: backendCoords[1],
      }
  ...
  }, [mapLoaded, step ,editingLocation]);
  ...
  // const isValid = location.address.length >= 5 && location.city && location.state && location.country || hasLocation;
  // const isValid = isEditMode && hasLocation ? true : (location.address.length >= 5 && location.city && location.state && location.country);
  // Handle next
  ...
  const handleNext = () => {
    ...
    if (step === 'pin' && isValid) {
      ...
      // router.push('/host/service/new/media');
       const next = goToNextSubStep();
    if (next) router.push(next);
    }
  };
